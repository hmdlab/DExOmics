...
def get_data_level(encode_X_mRNA_train, encode_X_mRNA_val, encode_X_promoter_train, encode_X_promoter_val, encode_Y_train, encode_Y_val, level='low'):
    """"Manipulate ENCODE data into different levels for doing the sanity check of the network"""
    if level == 'low':
        for i in range(len(encode_X_mRNA_train)):
            encode_X_mRNA_train.values[:,1][i][:] = 0
        for i in range(len(encode_X_promoter_train)):
            encode_X_promoter_train.values[:,1][i][:] = 0
        encode_Y_train[:] = 1

        for i in range(len(encode_X_mRNA_val)):
            encode_X_mRNA_val.values[:,1][i][:] = 0
        for i in range(len(encode_X_promoter_val)):
            encode_X_promoter_val.values[:,1][i][:] = 0
        encode_Y_val[:] = 1

    if level == 'medium':
        interval_len1 = int(len(encode_X_mRNA_train)/3)
        # mRNA 
        for i in range(len(encode_X_mRNA_train)):
            encode_X_mRNA_train.values[:,1][i][:] = 0
        # for i in range(0,interval_len1):
        #     encode_X_mRNA_train.values[:,1][i][:] = np.random.uniform(0, 5)
        # for i in range(interval_len1, interval_len1*2):
        #     encode_X_mRNA_train.values[:,1][i][:] = np.random.uniform(5, 10)
        # for i in range(interval_len1*2, len(encode_X_mRNA_train)):
        #     encode_X_mRNA_train.values[:,1][i][:] = np.random.uniform(10, 15)
        # promoter
        # for i in range(len(encode_X_promoter_train)):
        #     encode_X_promoter_train.values[:,1][i][:] = 0
        for i in range(0,interval_len1):
            encode_X_promoter_train.values[:,1][i][:] = np.random.uniform(0, 5)
            # encode_X_promoter_train.values[:,1][i][:] = 5
        for i in range(interval_len1, interval_len1*2):
            encode_X_promoter_train.values[:,1][i][:] = np.random.uniform(5, 10)
        for i in range(interval_len1*2, len(encode_X_promoter_train)):
            encode_X_promoter_train.values[:,1][i][:] = np.random.uniform(10, 15)
        encode_Y_train[0:interval_len1] = 0
        encode_Y_train[interval_len1:interval_len1*2] = 1
        encode_Y_train[interval_len1*2:len(encode_Y_train)] = 2

        interval_len2 = int(len(encode_X_mRNA_val)/3)
        # mRNA
        for i in range(len(encode_X_mRNA_val)):
            encode_X_mRNA_val.values[:,1][i][:] = 0
        # for i in range(0,interval_len2):
        #     encode_X_mRNA_val.values[:,1][i][:] = np.random.uniform(0, 5)
        # for i in range(interval_len2, interval_len2*2):
        #     encode_X_mRNA_val.values[:,1][i][:] = np.random.uniform(5, 10)
        # for i in range(interval_len2*2, len(encode_X_mRNA_val)):
        #     encode_X_mRNA_val.values[:,1][i][:] = np.random.uniform(10, 15)
        #promoter
        # for i in range(len(encode_X_promoter_val)):
        #     encode_X_promoter_val.values[:,1][i][:] = 0
        for i in range(0,interval_len2):
            encode_X_promoter_val.values[:,1][i][:] = np.random.uniform(0, 5)
        for i in range(interval_len2, interval_len2*2):
            encode_X_promoter_val.values[:,1][i][:] = np.random.uniform(5, 10)
        for i in range(interval_len2*2, len(encode_X_promoter_val)):
            encode_X_promoter_val.values[:,1][i][:] = np.random.uniform(10, 15)
        encode_Y_val[0:interval_len2] = 0
        encode_Y_val[interval_len2:interval_len2*2] = 1
        encode_Y_val[interval_len2*2:len(encode_Y_val)] = 2
        

    if level == 'high':
        for i in range(len(encode_X_mRNA_train)):
            encode_X_mRNA_train.values[:,1][i][:] = np.random.rand(1)
        for i in range(len(encode_X_promoter_train)):
            encode_X_promoter_train.values[:,1][i][:] = np.random.rand(1)
        encode_Y_train[:] = encode_Y_train

        for i in range(len(encode_X_mRNA_val)):
            encode_X_mRNA_train.values[:,1][i][:] = np.random.rand(1)
        for i in range(len(encode_X_promoter_val)):
            encode_X_promoter_val.values[:,1][i][:] = np.random.rand(1)
        encode_Y_val[:] = encode_Y_val
    return encode_X_mRNA_train, encode_X_mRNA_val, encode_X_promoter_train, encode_X_promoter_val, encode_Y_train, encode_Y_val